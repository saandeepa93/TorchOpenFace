...
FaceAnalyserParameters::FaceAnalyserParameters(std::vector<std::string> &arguments):root()
{
	...
	this->model_dir = fs::path(arguments[0]).filename().string();
	...
	if (dynamic)
	{
		this->model_location = this->model_dir + "/" + "AU_predictors/main_dynamic_svms.txt";
	}
	else
	{
		this->model_location = this->model_dir + "/" + "AU_predictors/main_static_svms.txt";
	}
