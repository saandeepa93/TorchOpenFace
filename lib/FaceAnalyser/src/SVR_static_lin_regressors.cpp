...
void SVR_static_lin_regressors::Predict(std::vector<double>& predictions, std::vector<std::string>& names, const cv::Mat_<double>& fhog_descriptor, const cv::Mat_<double>& geom_params)
{
	if(AU_names.size() > 0)
	{
		...
		else
		{
			...
			cv::Mat_<double> tmp1, tmp2, tmp3;
      tmp1 = (input - this->means);
      tmp2 = tmp1 * this -> support_vectors;
      tmp3 = tmp2 + this -> biases;
      // std::cout<<"fhog_descriptor: "<<fhog_descriptor.size()<<std::endl;
      // std::cout<<"input: "<<input.size()<<std::endl;
      // std::cout<<"this->means: "<<this->means.size()<<std::endl;
      // std::cout<<"this->support_vectors: "<<this->support_vectors.size()<<std::endl;
      // std::cout<<"this->biases: "<<this->biases.size()<<std::endl;
      // std::cout<<"geom_params: "<<geom_params<<std::endl;
      // std::cout<<"tmp1: "<<tmp1.size()<<std::endl;
